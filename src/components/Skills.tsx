                  ...
                  {data.skills.map((skill, skillIndex) => (
                    <div key={skill.name} className="group/skill relative">
                      <div className="relative overflow-hidden">
                        <Badge 
                          variant="secondary" 
                          className="w-full text-xs font-medium px-3 py-2 bg-muted/50 border border-border/50 text-foreground hover:bg-primary/20 hover:border-primary/40 hover:text-primary transition-all duration-300 professional-hover cursor-pointer hover:scale-105 justify-center"
                        ...
                        <div className="absolute bottom-0 left-0 h-0.5 bg-gradient-to-r from-primary to-accent transform scale-x-0 group-hover/skill:scale-x-100 transition-transform duration-300 origin-left" 
                             style={{ width: `${skill.level}%` }}
                        ></div>
                      </div>
                      
                      {/* Hover tooltip */}
                      <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 bg-card border border-border/50 px-2 py-1 rounded text-xs opacity-0 group-hover/skill:opacity-100 transition-opacity duration-200 pointer-events-none z-20 backdrop-blur-sm">
                        {skill.level}%
                      </div>
                    </div>
                  ))}
                ...
                <div className="pt-4 border-t border-border/30">
                  <div className="flex items-center justify-between text-xs">
                    ...
                    <span className="text-primary font-semibold">
                      {Math.round(data.skills.reduce((acc, skill) => acc + skill.level, 0) / data.skills.length)}%
                    </span>
                  </div>
                  <div className="w-full bg-muted/30 rounded-full h-2 mt-2 overflow-hidden">
                    <div 
                      ...
                      style={{ 
                        width: isVisible ? `${Math.round(data.skills.reduce((acc, skill) => acc + skill.level, 0) / data.skills.length)}%` : '0%',
